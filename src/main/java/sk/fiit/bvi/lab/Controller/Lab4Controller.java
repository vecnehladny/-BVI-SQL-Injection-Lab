...
public class Lab4Controller {
    ...
    public String login(@RequestParam(required = false) String profileId, @RequestParam(required = false) String password, HttpSession session, Model model) throws NoSuchAlgorithmException {
        String profileIdCurr = (String) session.getAttribute(Constants.PROFILE_ID_LAB_4);
        ...
        if(null != password) {
            ...
            if(!loginWrapper.getUsers().isEmpty()) {
                session.setAttribute(Constants.PROFILE_ID_LAB_4, profileId);
                model.addAttribute("users", loginWrapper.getUsers());
                if(loginWrapper.getUsers().size() > 1) {
                    model.addAttribute("lab4CTF", service.getCTF());
                }
    ...
    public String home(HttpSession session, Model model) {
        String profileId = (String) session.getAttribute(Constants.PROFILE_ID_LAB_4);
        if(null != profileId) {
    ...
    public String logout(HttpSession session) {
        String profileId = (String) session.getAttribute(Constants.PROFILE_ID_LAB_4);
        if(null != profileId) {
            session.removeAttribute(Constants.PROFILE_ID_LAB_4);
            return "lab4/login";
        }
        return "lab4/login";
    }
